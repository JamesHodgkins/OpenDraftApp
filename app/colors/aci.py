"""
Autodesk Color Index (ACI) — the standard 256-colour palette used by AutoCAD
and DXF-based applications.

Indices 0–9 are the "standard" named colours:
    0 = ByBlock, 1 = Red, 2 = Yellow, 3 = Green, 4 = Cyan, 5 = Blue,
    6 = Magenta, 7 = White/Black (context-dependent), 8 = Dark Grey, 9 = Light Grey.

Indices 10–249 form the main palette (hue/shade ramp).
Indices 250–255 are a greyscale ramp.

The mapping below stores (R, G, B) tuples keyed by index.
"""
from __future__ import annotations

from typing import Dict, Tuple

# Full ACI palette: index → (R, G, B)
...
def aci_to_rgb(index: int) -> Tuple[int, int, int]:
    """Return (R, G, B) for the given ACI index (1–255).

    Raises ``KeyError`` for invalid indices.
    """
    return ACI_COLORS[index]


def aci_to_hex(index: int) -> str:
    """Return a ``#RRGGBB`` hex string for the given ACI index."""
    r, g, b = ACI_COLORS[index]
    return f"#{r:02x}{g:02x}{b:02x}"


def hex_to_nearest_aci(hex_color: str) -> int:
    """Find the ACI index closest to a ``#RRGGBB`` hex string (Euclidean distance).

    Returns the best-matching ACI index (1–255).
    """
    ...
    best_idx = 7
    best_dist = float("inf")
    for idx, (r, g, b) in ACI_COLORS.items():
